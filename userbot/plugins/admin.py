...
async def promote(promt):
    ...
    CYBERevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await CYBERevent.edit("Promoted Successfully! Mje le bhnchoo...power ke 😂")
    except BadRequestError:
        await CYBERevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    CYBERevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await CYBERevent.edit(NO_PERM)
        return
    await CYBERevent.edit("Demoted Successfully!Bhut ud rhe the admin bn kr aa gyy zameen pe 😏")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    CYBERevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await CYBERevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await CYBERevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await CYBERevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    CYBERevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await CYBERevent.edit("Unbanned Successfully! Giving one more chance 😏")
        if BOTLOG:
        ...
        await CYBERevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    CYBERevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await CYBERevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await CYBERevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
