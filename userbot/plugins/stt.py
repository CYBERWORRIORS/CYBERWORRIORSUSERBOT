...
async def _(event):
    ...
    CYBERevent = await edit_or_reply(event, "Downloading to my local, for analysis  🙇")
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await CYBERevent.edit("Starting analysis, using IBM WatSon Speech To Text")
            headers = {
            ...
            if "results" in r:
                ...
                await CYBERevent.edit(string_to_show)
            else:
                await CYBERevent.edit(r["error"])
            # now, remove the temporary file
            os.remove(required_file_name)
    else:
        await CYBERevent.edit("Reply to a voice message, to get the relevant transcript.")
