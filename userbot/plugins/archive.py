...
from CYBERWORRIORSBOT.utils import admin_cmd, progress
...
async def _(event):
    ...
    await borg.send_file(
        event.chat_id,
        directory_name + ".zip",
        caption="Zipped By CYBERWORRIORSBOT",
        force_document=True,
        allow_cache=False,
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".rar",
                caption="rarred By CYBERWORRIORSBOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By CYBERWORRIORSBOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                output,
                caption="TAR By CYBERWORRIORSBOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
