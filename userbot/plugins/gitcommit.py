...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of CYBERWORRIORSBOT`")
        return
    CYBERWORRIORSBOT = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await CYBERWORRIORSBOT.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await CYBERWORRIORSBOT.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await CYBERWORRIORSBOT.edit("Committing to Github....")
        await git_commit(downloaded_file_name, CYBERWORRIORSBOT)


async def git_commit(file_name, CYBERWORRIORSBOT):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await CYBERWORRIORSBOT.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await CYBERWORRIORSBOT.edit("Cannot Upload Plugin")
    else:
        return await CYBERWORRIORSBOT.edit("`Committed Suicide`")
