...
async def _(event):
    ...
    CYBERevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sCYBER(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    CYBERUser = await event.client.get_me()
    if CYBERUser.username:
        curruser = CYBERUser.username
    else:
        curruser = "CYBERWORRIORSBOT"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    CYBERevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on CYBERWORRIORSBOT server and shows details'
).add_command(
