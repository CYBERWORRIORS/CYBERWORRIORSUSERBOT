...
from CYBERWORRIORSBOT.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(aura):
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(CYBER))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [CYBERWORRIORSBOT](https://t.me/CYBERSupport)",
            )
            await event.delete()
        except YouBlockedUserError:
