...
CYBERWORRIORSBOT = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_CYBERWORRIORSBOT_{pack}"
        packnick = (
            f"{CYBERWORRIORSBOT} Vol.{pack}"
            if CYBERWORRIORSBOT
            else f"@{user.username}'s CYBERWORRIORSBOT Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
