...
from CYBERWORRIORSBOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(CYBER):
    if CYBER.fwd_from:
        return
    reply_to_id = CYBER.message.id
    if CYBER.reply_to_msg_id:
        reply_to_id = CYBER.reply_to_msg_id
    event = await edit_or_reply(CYBER, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await CYBER.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await CYBER.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    CYBERreply = await event.get_reply_message()
    if not CYBERreply or not CYBERreply.media or not CYBERreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if CYBERreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    CYBERevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, CYBERreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await CYBERevent.edit("`This file is not supported`")
            CYBERresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            CYBERfile = Path(await event.client.download_media(CYBERresponse, "./temp/"))
            CYBERgif = Path(await unzip(CYBERfile))
            ...
            for files in (CYBERgif, CYBERfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await CYBERevent.edit("Unblock @tgstogifbot")
            return
