...
async def yardim(event):
    ...
    if tgbotusername is not None or CYBER_input == "text":
        results = await event.client.inline_query(tgbotusername, "@CYBER_Userbot")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of CYBERWORRIORSBOT\n\n"
        )
        CYBERcount = 0
        plugincount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=CYBERcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of the CYBERWORRIORSBOT are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=CYBERcount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                CYBERcount += 1
            await event.reply(
                string.format(count=CYBERcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            CYBERcount += 1
        await event.reply(string.format(count=CYBERcount), parse_mode="HTML")
