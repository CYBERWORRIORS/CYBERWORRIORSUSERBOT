...
async def bigspam(CYBER):
    if not CYBER.text[0].isalpha() and CYBER.text[0] not in ("/", "#", "@", "!"):
        CYBER_msg = CYBER.text
        CYBERWORRIORSBOT_count = int(CYBER_msg[9:13])
        CYBER_spam = str(CYBER.text[13:])
        for i in range(1, CYBERWORRIORSBOT_count):
            await CYBER.respond(CYBER_spam)
        await CYBER.delete()
        if LOGGER:
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what CYBERWORRIORSBOT iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
