...
@CYBERWORRIORSBOT.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@CYBERWORRIORSBOT.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {CYBER}\n\n⚡ **[CYBERary CYBERWORRIORSBOT](t.me/CYBER_Userbot)** ⚡"
...
@CYBERWORRIORSBOT.on(admin_cmd(pattern="install$", outgoing=True))
@CYBERWORRIORSBOT.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {CYBER}\n\n{string}\n\n        ⚡ **[CYBERary CYBERWORRIORSBOT](t.me/CYBER_Userbot)** ⚡", link_preview=False)
...
@CYBERWORRIORSBOT.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@CYBERWORRIORSBOT.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@CYBERWORRIORSBOT.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@CYBERWORRIORSBOT.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@CYBERWORRIORSBOT.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@CYBERWORRIORSBOT.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in CYBERWORRIORSBOT."
).add()
